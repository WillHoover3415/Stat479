...
If the 2024 MLB season were simulated many times, how often would the Los Angeles Dodgers come out on top? When account for the potential factors such as the ability of a starting pitcher 
...
If we were to simulate this season a large amount of times, how often would the Los Angeles Dodgers have the most wins? How often would they finish in the top 5? Was their league leading 98 wins a fluke? Or were the Dodgers truly a dominant force? 
...
This concept can be extended to a wide variety of outcomes, such as player salary, the specific batting order used, recent team performance, and many more. For this analysis, we will quantify the impact of a team's starting pitcher, their home field advantage, and their performance in recent games as a tool to predict the probability of that team winning a game versus a specific opponent. In light of the Dodgers winning their second straight World Series (sigh), we will use these models to question or confirm their continued regular season dominance. Was their recent dominance merely a fluke, or is the all star ensemble of baseball talent under performing?


One important thing to note for the remainder of this analysis is that there is a piece of missing data, this data being an incomplete game. This game took place between the Houston Astros and Cleveland Guardians on September 29, 2024. Following a lengthy rain delay, the game was cancelled and not re scheduled, due to the fact that neither team's playoff schedule would be affected by the outcome of said game. This game doesn't contain reliable pitching statistics, yet it can be effectively simulated by the estimated team strengths and recent performance, making it a defensible choice to include it in the model.

Also note that several times throughout this analysis, the `save` and `load` functions are used before or after the construction of certain cells. This is because the process of constructing these certain data sets was either complicated, time/memory consuming, or both. Thus, data is often loaded from the local machine after it was created and saved. Additionally, cells involved in the creation are often set to `eval = FALSE` to prevent a re-run and more memory stress.
...
The Bradley Terry model is a simple way of computing the probability of pairwise events, for example the likelihood of team i beating team j. For application to a league of p teams, each playing n games. The model assumes that each team $j = 1, ..., p$ has an underlying strength value $\lambda_j$, which is a value that represents the strength of team j compared to a reference team which has its strength set to 0. 

The probability that team i beats team j is defined as such. $$\mathbb{P}(\textrm{team i beats team j}) = \frac{e^{\lambda_{i}}}{e^{\lambda_{i}} + e^{\lambda_{j}}} = \frac{1}{1 + e^{-1 \times (\lambda_{i} - \lambda_{j})}}$$
...
Data incorporated in this analysis was sourced from baseball-reference.com, and fangraphs.com through the `baseballr` package, as well as player identification codes from (https://www.smartfantasybaseball.com/tools/). 

One of the premier tasks of a data scientist is properly cleaning, pre-processing, and preparing data before any form of advanced analysis can be performed. Pre-processing and cleaning proved to be both a crucial and important part of the project, due to several key factors. 

Unfortunately, we could not find a data frame formatted where every row was a game instance that contained information about both the game outcome and starting pitcher stats in a clean form, However, information was found for each subcategory, one for game results, and a second containing pitching information were recovered. Thus, a data frame containing info about the results and pitchers for each game was created by creating data frames for information about each game and outcome, and a second that recorded information about every starting pitcher outing for the season. Finally, the two data frames were combined to achieve the desired result. 

#### Game Logs 

To facilitate in the scraping of web data, a Statcast helper function called `annual_statcast_query` was implemented.
```{r}
...
Game logs were uncovered using the function `bref_team_results`, which scrapes individual game results for a specific team for a specific season.
...
One massive setback associated with this method is that the library containing all the players had many missing values for only the fan graphs ids, yet less missing ids than in the Chadwick Register. It was determined as the number of starts made in the run with just ids from the library was less than the total number of games played. 
...
Starting pitchers for the final run of the data set were identified
```{r}
...
```{r}
starting_pitchers_final_short = starting_pitchers_final %>% select(PlayerName, playerid, Date, Opp, teamid, Season, H_A, ERA, IP, ER, Home_Tm, Away_Tm, Date_Numeric)
...
With two data frames of equal size, we are now ready to combine these two frames into one representative data frame. Each row in the frame represents a single game, with information about each team, the outcome of the game, the home and away team and pitchers, as well as ERAS's, innings pitched, and earned runs of each pitcher. The challenge was devising a way to join the two data sets so that no data was lost. It was thus decided to break up the pitching data set into home and away pitchers, and then complete two separate joins.
...
home_pitchers = starting_pitchers_final_short %>%
  ...
  select(PlayerName, playerid, ERA, IP, ER, Date_Numeric, Home_Tm) %>%
...
away_pitchers = starting_pitchers_final_short %>%
  ...
  select(PlayerName, playerid, ERA, IP, ER, Date_Numeric, Away_Tm) %>%
...
As one might have noticed, throughout the data frame the same game appears twice. For example,  Thus, our final step is removing "duplicate" games, and this is done by simply selecting one instance of each game, in this case, where the home team is the team of interest corresponding to the original `game_log` data set. Columns are selected in a way such that a desired order in which they appear occurs.

```{r}
games_with_pitchers = game_log_with_pitchers %>%
  filter(Tm == Home_Tm) %>%
  arrange(Date_Numeric) %>%
  mutate(game_id = row_number()) %>% select(Date, Home_Tm, Away_Tm, Home_Winner, Away_Winner, Winner, Loser, Home_Runs, Away_Runs, Home_Pitcher, Home_Pitcher_ERA, Home_Pitcher_IP, Home_Pitcher_ER, Home_Pitcher_ID, Away_Pitcher, Away_Pitcher_ERA, Away_Pitcher_IP, Away_Pitcher_ER, Away_Pitcher_ID, Win_Pitch, Loss_Pitch, Save, cLI, Gm, Tm, DoubleHeader, Date_String, Date_Numeric, game_id, teamid)
...
The data was written as a CSV file for easier local storage and for convenient sharing between project teammates. This is the data that will be used to source the fitting of the co-variates based on home field advantage, as well as pitcher strength.
...
Now we are ready to fit the initial, baseline Bradley Terry Model. This version only considers the record of the team and their respective match ups. To fit the model, each possible match up is recorded and the total wins/losses for each match up is recorded.
...
The lambda values are then estimated. The Chicago Cubs are used as the reference team, although in practice it doesn't matter which team is held constant, as the lambda's for all other teams adjust in accordance and the same win/loss probabilities are yielded. 
...
The first step in this season simulation will be appending each team's lambda in accordance with each game in the data set. This step involves turning the output from the Bradley Terry model into a data frame. 
...
Now to "simulate" a season, we must replicate a data frame for the number of seasons we wish to replicate. Due to computation time, only 100 simulations are completed, as it takes considerable time ro replicate each season, as we create a whole new data frame for each simulation.
...
Various summary metrics are added, such as total wins/losses, as well as wins and losses while a given team is home or away. Teams are viewed from the perspective as if they are the home team.
...
```{r}
team_summary %>% filter(Home_Tm == "LAD")
```

Here, we compare the average wins per season across the simulated data frame compared to actual wins. 
...
team_summary = team_summary %>% left_join(home_winners, by = "Home_Tm") 
team_summary = team_summary %>% left_join(away_winners, by = c("Home_Tm" = "Away_Tm"))
```

```{r}
team_summary = team_summary %>% mutate(Wins = Home_Wins + Away_Wins, Losses = 162 - Wins) 
team_summary %>% select(Home_Tm, Average_Wins, Average_Losses, Wins, Losses)
```
...
Custom MLB color palette for improved visualization, attempts were made at converting an existing palette, but it only took full team names as arguments. Thus, a custom map was created for improved graphical readability.
...
```{r}
top10 = ggplot(data = team_summary, aes(x = reorder(Home_Tm, Times_Top_10), y = Times_Top_10, fill = Home_Tm)) + geom_col() + scale_fill_manual(values = mlb_colors_custom) + theme(legend.position = "none") + labs(x = "Team", y = "Top 10 Finishes Through 100 Simulations" )
top10
```

```{r}
top5 = ggplot(data = team_summary, aes(x = reorder(Home_Tm, Times_Top_5), y = Times_Top_5, fill = Home_Tm)) + geom_col()  + scale_fill_manual(values = mlb_colors_custom) + theme(legend.position = "none") + labs(x = "Team", y = "Top 5 Finishes Through 100 Simulations" )
top5
```


```{r}
top1 = ggplot(data = team_summary, aes(x = reorder(Home_Tm, Times_1st), y = Times_1st, fill = Home_Tm)) + geom_col() + scale_fill_manual(values = mlb_colors_custom) + theme(legend.position = "none") + labs(x = "Team", y = "Times First Place") + labs(x = "Team", y = "First Place Finishes Through 100 Simulations" )
top1
```
...
```{r}
records = ggplot(data = team_summary, aes(x = reorder(Home_Tm, Average_Wins), y = Average_Wins, fill = Home_Tm)) + geom_col() + scale_fill_manual(values = mlb_colors_custom) + theme(legend.position = "none") + labs(x = "Team", y = "Average Wins Through 100 Simulations" )
records
```


```{r}
ggplot(team_summary, aes(x = reorder(Home_Tm, Wins))) +
  geom_col(aes(y = Average_Wins), fill = "gray7", alpha = 0.6) +
  geom_col(aes(y = Wins, fill = Home_Tm), width = 0.5) +
...
One significant limitation of the model that must be considered is the variation among how long a starting pitcher plays for, as well as potential correlation between number of innings pitched and ERA. Generally, starting pitchers pitch less innings when they are giving up too many runs, and pitch longer when they are giving up fewer runs. This variability in innings also affects the portion of the game pitched However, there is potential that this implication is less important than it seems in the sense that "bad" starting pitchers tend to pitch for shorter times after allowing multiple runs, while "good" starting pitchers tend to play for longer, as they are less likely to allow runs.

Another considerable limitation of the model which considered starting pitchers was an inability to implement a feature which computed a delayed rolling average for ERA for each team. The intuition behind this feature is that for the first x, let's say 10 amount of games, a team's average ERA is not representative of the long run average over the 162 game season. To combat this, a delayed rolling average is employed which only starts calculating after the 10th game. Before this mark, the ERA's for each team are fixed at some reasonable, arbitrary constant;  a value representative of an average league ERA. Implementing this proved to be quite difficult, and it was ultimately decided to exclude this feature from the model. 
